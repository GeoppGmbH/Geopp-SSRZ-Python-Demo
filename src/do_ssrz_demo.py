...
# =============================================================================
def check_print_ssi_after_msg(dec_msg, print_ssi_after_msg):
...
def ssrz_demo(ssrz_file, user_llh, nav_file=None, dec_only=None,
              ...
              do_not_use_msg=[], do_not_use_gnss=[], print_ssi_after_msg=[],
    ...
    ***************************************************************************
    ...
    dec_out = open(dec_out_path, 'w')
    print(header, file=dec_out)
    if ((dec_only == 0) & (nav_file is not None)):
        if csv_out:
            ssi_out = open(os.path.join(out_folder, filename + '_ssi.csv'),
                           'w')
            print_ssi_after_msg = ['RT']
        else:
            ssi_out = open(os.path.join(out_folder, filename + '_ssi.txt'),
                           'w')
        ...
        ssi_dbg = None
        gvi_out = open(os.path.join(out_folder, filename + '_gvi.txt'), 'w')
                        ...
                        if ((int(read_msg.ssrz_msg_type[-3:]) < 11) &
                           ...
                           (dec_corr.tt is not None)):
                            ...
                            if ((nav_file is not None) and
                               ...
                               (dec_only != 1)):
                                p_ssi = True  # Process SSI flag
                                p_ssi = check_print_ssi_after_msg(dec_msg,
                                                                  print_ssi_after_msg)
                                if (not rec_lrm):
                                    p_ssi = False
                                    break
                                if week_start > 0 and week_end > 0:
                                    dtStart = trafo.diff_time_s(week_start,
                                                                time_start,
                                                                week, epoch)
                                    dtEnd = trafo.diff_time_s(week_end, time_end,
                                                              week, epoch)
                                    if dtStart < 0:
                                        p_ssi = False
                                    if dtEnd > 0:
                                        ...
                                        break
                                if p_ssi:
                                    osr = ssrz_ssr2osr.ssr2osr(ssr.corrections[-1],
                                                               dec_md, week,
                                                               epoch,
                                                               nav_data,
                                                               receiver,
